...
compute_survival_for_clone_TV_balance <- function(data,
                                          LB,
                                          UB,
                                           first.w=FALSE,
                                           first.cut.off=NA,
 ...
 # assign("df_long", df_long, envir = .GlobalEnv)
  ...
  if (first.w==TRUE){
    ...
    if(is.null(only.measur.full.time)){
      ...
      glm_censor_formula_num<- update(glm_censor_formula, . ~  ns(t, df =3))
      fit_glm0  <- glm(glm_censor_formula_num,
                       family = binomial(link = "logit"),
                       data   = data_censor_model)
  ...
  if (competing.risks) {
    ...
    return(list(surv_dat_long=surv_dat_long,data=df2))
  }
